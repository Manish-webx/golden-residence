...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (offcanvas) {
        ...
        // Close menu on link click handled by global anchor listener
    }
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            // Critical: If clicking from mobile menu, hide it and resume scroll
            ...
            if (bsOffcanvas && offcanvasNavbar.classList.contains('show')) {
                bsOffcanvas.hide();
                lenis.start(); // Ensure scrolling is not blocked
            }

            if (targetElem) {
                lenis.scrollTo(targetElem, {
                    duration: 1.5,
                    offset: -85, // Adjust for fixed header height
                    easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t))
                });
            }
        });
